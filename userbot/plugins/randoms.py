# Made By @SupRemE_AnanD. Keep Credit if you are gonna copy paste it. LOL NOOBS....
...
async def _(event):
    if event.fwd_from:
        return
    event = await edit_or_reply(event, "Making a shayri....")
    await asyncio.sleep(2)
...
async def _(event):
    if event.fwd_from:
        return
    event = await edit_or_reply(event, "Hey! Here's a fact about you..")
...
@bot.on(admin_cmd(pattern="gbye$", outgoing=True))
@bot.on(sudo_cmd(pattern="gbye$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    event = await edit_or_reply(event, "Hey! Read this and go🙂")
    await asyncio.sleep(2.3)
